...
//Note: max length is 28 because in german language, the link isn't rated text would go over 1 line at 29 chars
var recElemLength =
{
    maxLength           : 28,
    ...
    try
    {
        ...
        { 
            ...
            if (this._recommendationDefStyle.length == 0)
            {
                ...
                recStyle.push({});
                recStyle.push({});
                this.addRecommendationElemStyle(recStyle[recElemStyle.Small], "19px");  //make it 19px so we can fit text into 1 line in cases where chars > maxLength
                this.addRecommendationElemStyle(recStyle[recElemStyle.Normal], recElem.style.fontSize); //20px by default
            }
        }
...
mcafee_wa_balloondisplayer.prototype.setCategories = function (annoItem, config)
{
    ...
    for (var catIndex = 0; catIndex < 3; ++catIndex)
    {
        ...
        if (annoItem.categories.length > catIndex )
        {
